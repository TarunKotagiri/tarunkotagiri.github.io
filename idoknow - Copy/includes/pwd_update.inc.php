<?php
  ...
  if (!(empty($_POST['reset_pwd1'])|| trim($_POST['reset_pwd1'])=='') && !(empty($_POST['reset_pwd2'])|| trim($_POST['reset_pwd2'])=='')){
    if ($_POST['reset_pwd1']===$_POST['reset_pwd2']) {/*passwords match*/
      try{
        $db=new PDO('mysql:host=sql300.epizy.com;dbname=epiz_30863319_idoknow;charset=utf8','epiz_30863319','jjY9cWshPLtkA',array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
      }
      catch(Exception $err){
